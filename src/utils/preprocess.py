...
def preprocess_frames(x_full_frames, pic_size):
    frames_pil = []
    for frame in x_full_frames:
        # 确保帧不为空，并且具有有效的尺寸
        if frame is not None and frame.size > 0:
            try:
                resized_frame = cv2.resize(frame, (pic_size, pic_size))  # 尝试缩放
                frames_pil.append(Image.fromarray(resized_frame))  # 转换为 PIL 图像
            except Exception as e:
                print(f"Error resizing frame: {e}")
        else:
            print("Warning: Skipping empty or invalid frame.")  # 打印警告，跳过无效帧
...
class CropAndExtract():
    ...
    def generate(self, input_path, save_dir, crop_or_resize='crop', source_image_flag=False, pic_size=256):
        ...
        frames_pil = preprocess_frames(x_full_frames, pic_size)
        if len(frames_pil) == 0:
